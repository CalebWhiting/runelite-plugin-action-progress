...
public class UseItemOnItemDetector extends ActionDetector {
    ...
    private static final Ingredient PESTLE_AND_MORTAR = new Ingredient(
    ...
    public void onMenuOptionClicked(MenuOptionClicked evt) {
        ...
        Item[] items = IntStream.of(
                client.getSelectedItemIndex(),
                evt.getParam0()
        ).mapToObj(inventory::getItem).toArray(Item[]::new);

        for (Product product : PRODUCTS) {
            if (product.isMadeWith(items)) {
                int amount = product.getMakeProductCount(inventoryManager);

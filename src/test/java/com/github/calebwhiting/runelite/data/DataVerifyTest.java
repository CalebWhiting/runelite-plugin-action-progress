...
public class DataVerifyTest {
    ...
    public void verifyItemIdConstants() {
        verify(Fletching.class, "BOLT_TIPS", IDQuery.ofItems().query(".*_BOLT_TIPS?"));
        verify(Fletching.class, "ENCHANTED_BOLTS", IDQuery.ofItems().query(".*_BOLTS?_E"));
        verify(Fletching.class, "UNENCHANTED_BOLTS_AND_ARROWS",
                IDQuery.ofItems().query(
                        String.format("(%s)_(ARROW(S)?|BOLT(S)?|BRUTAL)", String.join("|", ARROW_TIP_MATERIALS))
                )
        );
        verify(Crafting.class, "SAPPHIRE_AND_OPAL_JEWELLERY", IDQuery.ofItems().query(
                "(SAPPHIRE|OPAL)_(RING|NECKLACE|BRACELET|AMULET)"
        ));
        verify(Crafting.class, "EMERALD_JEWELLERY", IDQuery.ofItems().query(
                "(EMERALD|JADE|PRENATURE)_(RING|NECKLACE|BRACELET|AMULET)"
        ));
        verify(Crafting.class, "TOPAZ_AND_RUBY_JEWELLERY", IDQuery.ofItems().query(
                "(RUBY|TOPAZ)_(RING|NECKLACE|BRACELET|AMULET)"
        ));
        verify(Crafting.class, "DIAMOND_JEWELLERY", IDQuery.ofItems().query(
                "DIAMOND_(RING|NECKLACE|BRACELET|AMULET)"
        ));
        verify(Crafting.class, "DRAGONSTONE_JEWELLERY", IDQuery.ofItems().query(
                "DRAGON(STONE)?_(RING|NECKLACE|BRACELET|AMULET)"
        ));
        verify(Crafting.class, "ONYX_JEWELLERY", IDQuery.ofItems().query(
                "ONYX_(RING|NECKLACE|BRACELET|AMULET)"
        ));
        verify(Crafting.class, "ZENYTE_JEWELLERY", IDQuery.ofItems().query(
                "ZENYTE_(RING|NECKLACE|BRACELET|AMULET)"
        ));
        verify(Crafting.class, "SILVER_AND_GOLD_ITEMS", IDQuery.ofItems()
                // Gold Jewellery
                .query(String.format("(%s)_(BRACELET|AMULET_U|NECKLACE|RING)", String.join("|", JEWELLERY_MATERIALS)))
                // Silver
                .query("UNSTRUNG_(SYMBOL|EMBLEM)|TIARA|SILVER_(SICKLE|BOLTS_UNF)")
                // Misc
                .query("CONDUCTOR|DEMONIC_SIGIL|SILVTHRILL_ROD")
        );
        verify(WintertodtDetector.class, "WOODCUTTING_ANIMATIONS", IDQuery.ofAnimations().query("WOODCUTTING_.*"));
    }
